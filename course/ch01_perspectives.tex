...
\ona{Before the two perspectives, the one theorem the whole course rests on. It says nothing about quantum computers or molecules: it is a fact about Hermitian matrices, and everything else is about which vectors $\ket\psi$ one can afford to try.}
...
\begin{thm}[Variational principle]\label{thm:ch01:variational}
...
\begin{frame}\ft{Why molecules? The problem, in computer-science terms}
\onp{\footnotesize}
\ona{You need no chemistry for this course. What you need is the following computational problem.}
\begin{itemize}\setlength\itemsep{0.3em}
    \item \textbf{Input:} a molecule, i.e., a list of atoms with their positions in space (and a desired accuracy).
    \item \textbf{Output:} its lowest possible energy $E_0$, a single real number, and ideally the state that achieves it.
    \item \textbf{Why anyone cares:} differences of such energies decide whether a reaction happens, how strongly a drug binds to its target, whether a material conducts, what colour it has. The standard target, ``chemical accuracy'', is $1.6\times10^{-3}$ in the natural unit (Hartree), or $1$ kcal/mol; it is what predicts a reaction rate to within a factor of ten.
    \item \textbf{Why it is hard:} $E_0$ is the smallest eigenvalue of a Hermitian matrix whose dimension is exponential in the number of electrons; a large share of the world's supercomputing time goes into approximating it.
    \item \textbf{Why quantum computers:} $n$ qubits store a vector of $2^n$ amplitudes natively. Simulating molecules was the first proposed application of quantum computers (Feynman 1982; a concrete algorithm in 2005), and the variational quantum eigensolver of \citet{peruzzo2014variational} was the first algorithm run on hardware for it.
\end{itemize}
\end{frame}

\begin{frame}\ft{The smallest molecule}
\ona{We use the smallest example throughout: molecular hydrogen, H$_2$. It consists of two protons (positively charged, heavy) at a distance $R$ apart, and two electrons (negatively charged, light) that are attracted to both protons and repelled by each other. The only free parameter is $R$; the equilibrium distance is $R_{\rm eq} \approx 0.74$~\angstrom{} ($10^{-10}$ metres). Figure~\ref{fig:ch01:h2mol} shows the two protons and where the electrons are likely to be found: between them, which is what holds the molecule together.}
\begin{center}
...
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{Schematic of H$_2$: two protons at distance $R$ and the region where the two electrons are most likely to be found in the lowest-energy state.}\label{fig:ch01:h2mol}\end{figure}}
\ona{Our task: compute the lowest energy of this system as a function of $R$. Everything that follows in this course, including the QAOA circuits on hundreds of qubits, descends from this question.}
\onp{Two protons at distance $R$, two electrons. Task: the lowest energy as a function of $R$.}
\end{frame}

\begin{frame}\ft{From a molecule to a Hermitian matrix, in four steps}
\onp{\scriptsize}
\ona{Physics supplies an energy operator (the \emph{Hamiltonian}) that accounts for the electrons' kinetic energy, their attraction to the protons and their mutual repulsion. Turning it into a matrix a computer can handle takes four standard steps:}
\begin{enumerate}\setlength\itemsep{0.25em}
    \item \textbf{Freeze the nuclei.} Protons are 2000 times heavier than electrons, so their positions are treated as fixed parameters (here, $R$). Only the electrons are quantum.
    \item \textbf{Discretise space.} Choose $M$ basis functions of position, called \emph{orbitals}, in which an electron can sit, much as a finite-element method chooses a mesh. Chemists have good, small bases: for H$_2$ the minimal basis has one function per atom.
    \item \textbf{Encode configurations as bit strings.} Each electron also has a spin (up or down), so there are $2M$ ``slots''. A configuration of $N$ electrons is a bit string of length $2M$ with $N$ ones: slot occupied or not. These strings form the basis of the state space, and a bit string is a computational basis state of $2M$ qubits.
    \item \textbf{Compute the matrix.} In this basis the Hamiltonian is a Hermitian $2^{2M}\times2^{2M}$ matrix $H$ whose entries are integrals over the orbitals, computed classically in polynomial time. It is highly structured: a sum of $\mathcal O(M^4)$ Pauli strings (\Chap~\chref{C.VQE}).
\end{enumerate}
\ona{For H$_2$ in the minimal basis, $M = 2$ and $N = 2$, so four qubits; using two symmetries (spin and mirror symmetry of the molecule), this reduces to two qubits and a $4\times4$ matrix \cite{omalley2016scalable}:}
\begin{equation}\label{eq:ch01:h2qubit}
    H(R) = g_0\,\one + g_1 Z_0 + g_2 Z_1 + g_3 Z_0Z_1 + g_4 X_0X_1 + g_5 Y_0Y_1,
\end{equation}
\ona{six real numbers $g_k(R)$ for every bond length $R$. All numbers below come from exact diagonalisation in this basis, computed with PySCF by \texttt{code/ch01\_h2.py}.}
\onp{H$_2$, minimal basis: $M = 2$ orbitals, $N = 2$ electrons, four qubits, two after symmetries: a $4\times4$ matrix with six coefficients $g_k(R)$ \cite{omalley2016scalable}.}
\end{frame}

\begin{frame}\ft{The lowest eigenvector: the wave function}
\onp{\footnotesize}
\ona{An orbital is just a function of position. In the minimal basis of H$_2$ the two orbitals are the sum and the difference of the two atomic ones, shown along the line joining the protons in Fig.~\ref{fig:ch01:h2wf}: the sum piles amplitude between the protons (\emph{bonding}, lower energy), the difference has a node there (\emph{antibonding}, higher energy).}
\ona{The cheapest guess for the lowest eigenvector is a single basis state: both electrons, one of each spin, in the bonding orbital. Chemists call it the \emph{Hartree--Fock} state; in the two-qubit encoding \eqref{eq:ch01:h2qubit} it is $\ket{10}$, and its energy at $R = 0.75$~\angstrom{} is $-1.116$ Hartree. The true lowest eigenvector, of energy $-1.137$, is a superposition with the state in which both electrons sit in the antibonding orbital:}
\begin{equation}\label{eq:ch01:h2gs}
    \ket{\psi_0} = \cos\tfrac\theta2\,\ket{10} - \sin\tfrac\theta2\,\ket{01},\qquad \theta = 0.23,\ 0.73,\ 1.50 \text{ at } R = 0.75,\ 1.5,\ 3.0\,\angstrom.
\end{equation}
\ona{A single rotation angle $\theta$ describes it exactly: this one-parameter circuit was the first variational quantum eigensolver run on hardware \cite{omalley2016scalable}. As the protons are pulled apart, $\theta \to \pi/2$ and the single-basis-state guess becomes badly wrong.}
\onp{Cheapest guess: one basis state $\ket{10}$ (Hartree--Fock), energy $-1.116$. Truth: $\cos\tfrac\theta2\ket{10} - \sin\tfrac\theta2\ket{01}$, energy $-1.137$; $\theta$ grows from $0.23$ to $1.50$ as the atoms separate.}
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=0.72\linewidth,height=0.3\linewidth,xlabel={position along the bond axis (Bohr)},ylabel={amplitude},xmin=-4,xmax=4,ymin=-1.1,ymax=1.3,axis lines=left,legend style={font=\scriptsize,draw=none,at={(0.02,0.98)},anchor=north west},legend columns=2,samples=200,domain=-4:4,no markers,every axis plot/.append style={thick}]
...
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{The two atomic basis functions on the two protons (dots) and their sum (bonding) and difference (antibonding) along the line joining the protons; schematic amplitudes.}\label{fig:ch01:h2wf}\end{figure}}
\end{frame}

\begin{frame}\ft{The eigenvalues: energy levels}
\ona{Diagonalising the $4\times4$ matrix \eqref{eq:ch01:h2qubit} at $R = 0.75$~\angstrom{} gives four eigenvalues, the \emph{energy levels} of Fig.~\ref{fig:ch01:h2levels}. The lowest is what we want. The single-basis-state guess lies $0.021$ Hartree above it, thirteen times chemical accuracy: this \emph{correlation energy}, the effect of the superposition, is what any method, classical or quantum, must recover.}
\begin{center}
\begin{tikzpicture}[x=1cm,y=2.4cm,every node/.style={font=\scriptsize}]
\draw[->] (0,-1.3) -- (0,0.65) node[above] {$E$ (Hartree)};
\foreach \y in {-1.0,-0.5,0,0.5} {\draw (-0.08,\y) -- (0.08,\y) node[left=6pt] {\y};}
\draw[very thick,blue!70!black] (1,-1.1371) -- (3.4,-1.1371) node[right,yshift=-5pt] {$E_0 = -1.137$: singlet ground state};
\draw[thick,dashed,gray] (1,-1.1162) -- (3.4,-1.1162) node[right,gray,yshift=5pt] {Hartree--Fock $\ket{10}$: $-1.116$};
\draw[very thick,red!70!black] (1,-0.5428) -- (3.4,-0.5428) node[right] {$E_1 = -0.543$};
\draw[very thick,red!70!black] (1,-0.1792) -- (3.4,-0.1792) node[right] {$E_2 = -0.179$};
\draw[very thick,red!70!black] (1,0.4598) -- (3.4,0.4598) node[right] {$E_3 = 0.460$: doubly excited};
\draw[<->,thick] (0.6,-1.1371) -- (0.6,-0.5428) node[midway,left] {$\gamma = 0.59$};
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{Energy levels of H$_2$ in the minimal basis at $R = 0.75$~\angstrom{}, exact diagonalisation. The dashed line is the energy of the best single basis state (Hartree--Fock); its distance to $E_0$ is the correlation energy.}\label{fig:ch01:h2levels}\end{figure}}
\ona{Three things to retain, because the whole course turns on them: (i) we want the lowest eigenvalue and its eigenvector; (ii) a cheap guess is close in energy and has large \emph{overlap} with the truth, $|\braket{10|\psi_0}|^2 = \cos^2(\theta/2) = 0.987$; (iii) there is a \emph{gap} $\gamma$ between the lowest eigenvalue and the next. Overlap and gap are the two quantities on which the guarantees of the following slides, and of \Chap~\chref{C.EQA}, depend.}
\onp{Retain: lowest eigenvalue wanted; cheap guess has overlap $0.987$ with the truth; gap $\gamma = 0.59$ to the next level. Guarantees below depend on overlap and gap.}
\end{frame}

\begin{frame}\ft{Repeat for every $R$: the potential-energy curve}
\ona{Doing the same for every distance $R$ gives the curves of Fig.~\ref{fig:ch01:h2curves}. The minimum of the lowest curve is the bond length ($0.71$~\angstrom{} in this small basis, $0.74$ experimentally); its curvature gives the frequency at which the molecule vibrates; its depth, the energy needed to break the bond. The single-basis-state guess is fine near the minimum and wrong by $0.28$ Hartree when the atoms are far apart: there, one electron must sit on each atom, which no single basis state of this kind describes, and only the superposition does. Chemists call such states \emph{strongly correlated}; the overlap of the cheap guess with the truth falls from $0.99$ to $0.54$, and this is the regime in which quantum algorithms are hoped to help.}
\onp{Minimum $=$ bond length; curvature $=$ vibration frequency; depth $=$ bond energy. The cheap guess fails at large $R$ (overlap $0.99 \to 0.54$): ``strongly correlated'', where quantum help is hoped for.}
\begin{center}
\begin{tikzpicture}
\begin{axis}[width=0.72\linewidth,height=0.42\linewidth,xlabel={bond length $R$ (\angstrom)},ylabel={energy (Hartree)},xmin=0.4,xmax=3,ymin=-1.2,ymax=0.7,legend style={font=\scriptsize,draw=none,at={(0.98,0.45)},anchor=east},thick,no markers]
\addplot[blue!70!black] table[x=R,y=E0] {figures/h2/h2_curves.dat};
\addplot[gray,dashed] table[x=R,y=E_HF] {figures/h2/h2_curves.dat};
\addplot[red!70!black] table[x=R,y=E1] {figures/h2/h2_curves.dat};
\addplot[red!70!black,dotted] table[x=R,y=E2] {figures/h2/h2_curves.dat};
\legend{$E_0$ exact (FCI),Hartree--Fock,$E_1$,$E_2$}
\end{axis}
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{Energy levels of H$_2$ in the minimal basis as a function of the distance $R$ between the protons: exact lowest and excited eigenvalues, and the energy of the best single basis state (Hartree--Fock). Computed with PySCF by \texttt{code/ch01\_h2.py}.}\label{fig:ch01:h2curves}\end{figure}}
\end{frame}

\begin{frame}\ft{Is there an exponential quantum advantage here?}
\ona{H$_2$ is easy for any method. The question that matters is about \emph{scaling}: as molecules get bigger, and the matrix size $L$ grows with the number of atoms, does a quantum computer find the lowest eigenvalue exponentially faster than the best classical program? This is the \emph{exponential quantum advantage} (EQA) hypothesis, examined critically by \citet{lee2023evaluating}, whom we follow here and in \Chap~\chref{C.EQA}. Their accounting of the running time of the standard quantum algorithm (phase estimation) to precision $\epsilon$ is}
\begin{equation}\label{eq:ch01:qpecost}
    \mathrm{poly}(1/S)\,\big[\mathrm{poly}(L)\,\mathrm{poly}(1/\epsilon) + C\big], \qquad S = |\braket{\Phi|\Psi_0}|,
\end{equation}
\ona{where $C$ is the cost of preparing a starting guess $\Phi$, the bracket is the quantum circuit, and the prefactor, $1/S^2$ for plain phase estimation, is the number of repetitions needed: the algorithm returns the lowest eigenvalue only with probability $S^2$, the overlap of the guess with the true eigenvector, so one repeats until it does. The $\mathrm{poly}(L)$ in the middle is what is usually quoted. EQA requires, in addition, that the overlap $S$ does not shrink exponentially with $L$, \emph{and} that every classical program costs $\exp(L)$ on the same molecules.}
\onp{\scriptsize EQA hypothesis \cite{lee2023evaluating}: for generic chemistry, quantum polynomial \emph{and} classical exponential in $L$. In the QPE cost, the overlap $S$ of the trial state is the crux.}
\figslide[height=0.36\textheight]{figures/eqa/fig1.pdf}{Ansatz state preparation for iron--sulphur clusters, from \citet{lee2023evaluating}. (A) [2Fe-2S], [4Fe-4S], the P-cluster and the FeMo cofactor of nitrogenase, up to 154 qubits. (B) Overlap $S^2$ of the best cheap starting guess (a single basis state, purple; a symmetry-adapted combination, orange) with the exact lowest eigenvector, against the number of metal atoms: an exponential decrease, down to $10^{-7}$ for FeMo-co.}{fig:ch01:eqa1}
\end{frame}

\begin{frame}\ft{And the classical competition does not scale exponentially}
\ona{The second requirement of EQA fails empirically too: for the molecules usually cited as motivation, classical heuristics reach chemical accuracy in polynomial time. The workhorse method of quantum chemistry (\emph{coupled cluster}, a clever low-parameter approximation of the eigenvector) has an empirical cost polynomial in $1/\epsilon$ and, because interactions are local, linear in the molecule size for ordinary molecules such as alkane chains, with constant error per atom; it runs routinely on thousands of electrons. For the hardest, strongly correlated model systems, tensor-network methods (the same matrix product states we meet in \Chap~\chref{C.PerIteration}) show near-linear cost in size and error decreasing polynomially in their expressiveness parameter $D$. \citet{lee2023evaluating} conclude that evidence for a \emph{generic} exponential advantage has yet to be found, while polynomial speedups remain possible and would still be valuable.}
\figslide[height=0.45\textheight]{figures/eqa/fig3.pdf}{Computational complexity of classical heuristics for molecules, from \citet{lee2023evaluating}. (A) Error of the coupled-cluster hierarchy on the nitrogen molecule against running time: polynomial in $1/\epsilon$. (B) Running time of a locality-exploiting coupled-cluster code on alkane chains C$_m$H$_{2m+2}$ with up to $m = 120$ carbons: nearly linear in size, with constant error per carbon. (C) A protein fragment with thousands of electrons, treated on a few processor cores.}{fig:ch01:eqa3}
\onp{\scriptsize Classical heuristics: $\mathrm{poly}(1/\epsilon)$ and near-linear in $L$ where they apply. Verdict \cite{lee2023evaluating}: no evidence yet of generic EQA; polynomial speedups still possible. Details in \Chap~\chref{C.EQA}.}
\end{frame}

\begin{frame}\ft{The general recipe, and where quantum algorithms enter}
\ona{For a general molecule the four steps give an $M$-orbital, $2M$-qubit problem with a Hamiltonian of $\mathcal O(M^4)$ terms; the molecules people care about need $50$ to $150$ qubits (the FeMo cofactor of nitrogenase, which fixes nitrogen from the air, is $154$ qubits in a reduced model). Three ways to get at the lowest eigenvalue:}
\begin{itemize}\setlength\itemsep{0.25em}
    \item \textbf{Exact classical diagonalisation:} exponential in $M$; feasible to about $20$ orbitals.
    \item \textbf{Classical heuristics} (coupled cluster, tensor networks, quantum Monte Carlo): polynomial, excellent for most molecules, failing for some strongly correlated ones.
    \item \textbf{Quantum phase estimation:} given a starting guess with overlap $\eta$ with the true eigenvector, returns $E_0$ to precision $\epsilon$ using $\widetilde{\mathcal O}(1/(\eta\,\epsilon))$ steps of a quantum simulation of $H$, but needs deep, error-corrected circuits.
    \item \textbf{The variational route}, this course's subject: guess a family of circuits, minimise the energy over its parameters, using the variational principle of the next section. Cheap circuits, no guarantee.
\end{itemize}
\onp{$2M$ qubits, $\mathcal O(M^4)$ terms, $50$--$150$ qubits for interesting molecules. Exact: exponential. Heuristics: polynomial, usually fine. Phase estimation: $\widetilde{\mathcal O}(1/(\eta\epsilon))$, needs error correction. Variational: cheap, no guarantee.}
\end{frame}

\begin{frame}\ft{Beyond the energy: properties of the lowest eigenvector}
\ona{Applications often need more than $E_0$: where the electric charge sits (dipole moments), how electrons move through a material, the forces on the nuclei. Each is an expectation value $\braket{\psi_0|O|\psi_0}$ of some Hermitian matrix $O$ in the lowest eigenvector. \citet{zhang2022computing} formalise this as \emph{ground-state property estimation}: given $H$ with gap $\gamma$, an observable $O$, a circuit preparing a guess $\ket{\phi_0}$ with overlap $|\braket{\phi_0|\psi_0}|^2 \ge \eta$, and a precision $\epsilon$, estimate $\braket{\psi_0|O|\psi_0}$ to within $\epsilon$. Without the overlap and gap assumptions this is provably harder than estimating the energy.}
\begin{itemize}\setlength\itemsep{0.25em}
    \item \textbf{Variational route:} optimise a circuit to approximate $\ket{\psi_0}$, then measure $O$. Cheap circuits; but no guarantee on how close the optimised state is, hence none on the property.
    \item \textbf{Early fault-tolerant route} \cite{zhang2022computing}: use the guess only as a starting point with overlap $\eta$, and extract $\braket{\psi_0|O|\psi_0}$ from many short circuits plus classical Fourier post-processing. Longest circuit $\widetilde{\mathcal O}(\gamma^{-1})$, independent of $\epsilon$; total work $\widetilde{\mathcal O}(\gamma^{-1}\epsilon^{-2}\eta^{-2})$; one extra qubit. \emph{Guaranteed} accuracy, whatever the quality of the guess, paid for in running time.
\end{itemize}
\ona{The two routes are complementary: the variational one supplies the starting guess, the other supplies the guarantee. This is the role we assign to variational methods in chemistry in \Chaps~\chref{C.EQA} and~\chref{C.VQE}.}
\end{frame}

\begin{frame}\ft{How the guarantee works, in one slide (optional)}
\ona{Here is the idea, for the curious; nothing later depends on it. Write the starting guess in the eigenbasis of $H$, $\ket{\phi_0} = \sum_k\alpha_k\ket{\psi_k}$, and think of $p_k = |\alpha_k|^2$ as a probability distribution over the energy levels $\lambda_k$. Its cumulative distribution function $C(x) = \sum_{\lambda_k\le x}p_k$ is a staircase whose first step is at the lowest eigenvalue $E_0$ and has height $p_0 \ge \eta$. Lin and Tong \cite{lt21} observe that the quantum computer can measure the \emph{Fourier transform} of this distribution:}
\begin{equation}\label{eq:ch01:acdf}
    \braket{\phi_0|e^{-ijH}|\phi_0} = \sum_kp_ke^{-ij\lambda_k}
\end{equation}
\ona{one number for each integer time $j$, each estimated by a small circuit (a Hadamard test: one extra qubit controlling the evolution $e^{-ijH}$). From finitely many Fourier coefficients, $|j| \le d$, one reconstructs a smoothed staircase $\tilde C$ accurate to within $\eta/8$ everywhere except within $\delta \sim 1/d$ of a step. A binary search for the first point where $\tilde C \ge \eta/2$ then finds $E_0$ to precision $\delta$. Weighting the same construction by an observable $O$ gives $p_0\braket{\psi_0|O|\psi_0}$, and dividing by $p_0$ gives the property \cite{zhang2022computing}. The longest circuit has length $d \sim 1/\gamma$, set by the gap, not by the final precision; the price of precision is paid in repetitions, $\widetilde{\mathcal O}(\epsilon^{-2}\eta^{-2})$ of them.}
\onp{View the starting guess as a distribution over energy levels; small circuits measure its Fourier coefficients; reconstruct the CDF; its first step is $E_0$ (Lin--Tong); weight by $O$ for properties (Zhang--Wang--Johnson). Circuit length $\sim 1/\gamma$; precision paid in repetitions.}
\end{frame}
...
\ona{With the variational principle (Theorem~\ref{thm:ch01:variational}) in hand, we can now say what a variational quantum algorithm is, on both sides of the table above.}
...
\node[align=center,text width=3.4cm] at (0,-1.95) {(a) optimum: $4$ of $5$ edges cut};
\node at (0,-2.4) {$\mathrm{OPT} = 4$};
\end{scope}
% (b) vector relaxation: pentagram
\begin{scope}[xshift=4.6cm]
\draw[gray!60] (0,0) circle (1.1);
\foreach \i in {0,...,4}{
  \draw[-{Stealth[length=4pt]},thick,blue!70!black] (0,0) -- ({90+144*\i}:1.1) node[pos=1.2,font=\tiny] {$v_{\i}$};}
...
\node[align=center,text width=3.6cm] at (0,-1.95) {(b) unit vectors, adjacent ones $144^\circ$ apart};
\node at (0,-2.4) {$\mathrm{SDP} = \tfrac52(1-\cos\tfrac{4\pi}5) \approx 4.52$};
\end{scope}
% (c) SDP Gram matrix
...
\addplot3[surf,fill opacity=0.25,color=orange,domain=-0.95:-0.05,y domain=-0.95:-0.05,samples=2,samples y=2,shader=flat,faceted color=orange!80!black] {-1.5 - x - y};
...
\node[red!70!black] at (142:1.7) {arc $\theta_{ij}$};
\node[red!70!black] at (322:1.7) {arc $\theta_{ij}$};
...
\node[align=center,text width=4.6cm] at (0,-2.15) {$\mathrm{Pr}[\text{cut}] = 2\theta_{ij}/2\pi$: the hyperplane separates $v_i,v_j$ iff $\bar r$ falls in a red arc};
\end{scope}
...
\node[align=center,text width=4.6cm] at (0,-2.15) {a random hyperplane splits the pentagram: here into a $4$-edge cut};
\end{scope}
\end{tikzpicture}
\end{center}
...
\onp{\end{column}\begin{column}{0.3\textwidth}\centering
\qrcode[height=2.6cm]{https://github.com/jmarecek/variational-algorithms/tree/main/course/code}\\[2pt]
{\tiny\texttt{github.com/jmarecek/}\\\texttt{variational-algorithms}\\\texttt{course/code}}
\end{column}\end{columns}}
\ona{\begin{center}\qrcode[height=2.2cm]{https://github.com/jmarecek/variational-algorithms/tree/main/course/code}\end{center}}
\ona{\begin{center}\footnotesize The code of this course: \url{https://github.com/jmarecek/variational-algorithms/tree/main/course/code}\end{center}}
\end{frame}
