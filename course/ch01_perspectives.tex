...
\ona{Molecular hydrogen, H$_2$: two protons at distance $R$ ($R_{\rm eq} \approx 0.74$~\angstrom{} $= 1.40$ Bohr) and two electrons. Figure~\ref{fig:ch01:h2mol} shows the nuclei and the electron density of the bonding state, concentrated between the nuclei.}
\begin{center}
...
\end{equation}
\ona{with coefficients $g_k(R)$ computed classically from the one- and two-electron integrals for each bond length $R$, and tabulated, e.g., in \citet{omalley2016scalable}. All numbers below are exact diagonalisations in the minimal (STO-3G) basis, computed with PySCF by \texttt{code/ch01\_h2.py}; the nuclear repulsion $1/R$ is included.}
...
\ona{In the minimal basis the molecular orbitals are the symmetric and antisymmetric combinations of the atomic $1s$ orbitals $\phi_{A,B}(\boldsymbol r) \propto e^{-|\boldsymbol r - \boldsymbol R_{A,B}|}$, shown along the bond axis in Fig.~\ref{fig:ch01:h2wf}. Hartree--Fock puts both electrons (spin up and down) into $\sigma_g$: in the two-qubit encoding of \eqref{eq:ch01:h2qubit} this is the basis state $\ket{10}$, of energy $-1.116$ Hartree at $R = 0.75$~\angstrom. The exact ground state, of energy $-1.137$ Hartree, mixes in the doubly excited configuration $\ket{01}$,}
...
\end{equation}
\ona{which is exactly the one-parameter unitary coupled-cluster ansatz $e^{-i\theta/2\,(X_0Y_1 - Y_0X_1)/2}\ket{10}$: a variational circuit with a single angle, and the first VQE ever run on hardware \cite{omalley2016scalable}. The mixing grows with $R$ and tends to $\theta \to \pi/2$ at dissociation, where Hartree--Fock fails by $0.28$ Hartree.}
\begin{center}
\begin{tikzpicture}
...
\ona{Diagonalising \eqref{eq:ch01:h2qubit} at $R = 0.75$~\angstrom{}, equivalently full configuration interaction in the minimal basis, gives four energy levels, Fig.~\ref{fig:ch01:h2levels}. The ground state is the correlated singlet \eqref{eq:ch01:h2gs}; Hartree--Fock lies $0.021$ Hartree above it, thirteen times chemical accuracy: this correlation energy is what a variational circuit must recover. The other levels are excited configurations.}
\begin{center}
...
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{Energy levels of H$_2$ in the minimal basis at $R = 0.75$~\angstrom{} (full configuration interaction, STO-3G, nuclear repulsion included). The dashed line is the Hartree--Fock energy; the difference to $E_0$ is the correlation energy.}\label{fig:ch01:h2levels}\end{figure}}
\ona{Three things to retain: the ground state is what we want; a cheap trial state (Hartree--Fock) is close to it in energy and has large overlap with it, $|\braket{10|\psi_0}|^2 = \cos^2(\theta/2) = 0.987$; and there is a spectral gap $\gamma$ to the next level. Overlap and gap are the two quantities on which the guarantees of the following slides, and of \Chap~\chref{C.EQA}, depend.}
\onp{Overlap of Hartree--Fock with the ground state $0.987$; gap $\gamma = 0.59$ Hartree: the two quantities the guarantees below depend on.}
\end{frame}

\begin{frame}\ft{Energy levels as a function of bond length}
\ona{Repeating the diagonalisation for every $R$ gives the potential-energy curves of Fig.~\ref{fig:ch01:h2curves}: the minimum of the ground-state curve is the bond length ($0.71$~\angstrom{} in this basis, $0.74$ experimentally), its curvature the vibrational frequency, and its depth the dissociation energy. Hartree--Fock is accurate near equilibrium and wrong by $0.28$ Hartree at dissociation, where the two electrons must localise on different atoms, a strongly correlated state; the overlap of the trial state with the truth drops from $0.99$ to $0.54$, and this is the regime in which quantum algorithms are hoped to help.}
\begin{center}
\begin{tikzpicture}
...
\end{tikzpicture}
\end{center}
\ona{\begin{figure}[htb]\caption{Potential-energy curves of H$_2$ in the STO-3G basis: exact ground and excited states and the Hartree--Fock energy, computed with PySCF by \texttt{code/ch01\_h2.py}.}\label{fig:ch01:h2curves}\end{figure}}
\end{frame}

\begin{frame}\ft{Is there an exponential quantum advantage here?}
\ona{H$_2$ in a minimal basis is trivial for any method. The question that matters is asymptotic: for a family of ever larger molecules, with basis size $L$ growing with the number of atoms, does a quantum computer compute the ground-state energy exponentially faster than the best classical method? This is the \emph{exponential quantum advantage} (EQA) hypothesis, examined critically by \citet{lee2023evaluating}, whom we follow here and in \Chap~\chref{C.EQA}. Their accounting of the cost of quantum phase estimation to precision $\epsilon$ is}
...
\end{equation}
\ona{where $C$ is the cost of preparing the trial state $\Phi$, the bracket is the phase-estimation circuit, and the prefactor, $1/S^2$ for plain phase estimation, is the number of repetitions needed to land on the ground state rather than another eigenstate. The $\mathrm{poly}(L)$ in the middle is what is usually quoted. EQA requires, in addition, that $S$ does not decay exponentially with $L$, \emph{and} that every classical heuristic costs $\exp(L)$ on the same molecules.}
...
\figslide[height=0.36\textheight]{figures/eqa/fig1.pdf}{Ansatz state preparation for iron--sulphur clusters, from \citet{lee2023evaluating}. (A) [2Fe-2S], [4Fe-4S], the P-cluster and the FeMo cofactor of nitrogenase, up to 154 qubits. (B) Weight $S^2$ of the best single determinant (purple) and of the best configuration state function (orange) in the exact ground state, against the number of metal centres: an exponential decrease, down to $10^{-7}$ for FeMo-co.}{fig:ch01:eqa1}
\end{frame}

\begin{frame}\ft{And the classical competition does not scale exponentially}
\ona{The second requirement of EQA fails empirically too: for the molecules usually cited as motivation, classical heuristics reach chemical accuracy at polynomial cost. Coupled-cluster methods have an empirical precision cost $\mathrm{poly}(1/\epsilon)$ and, exploiting locality, a cost linear in $L$ on gapped systems such as alkanes, with constant error per subunit; they run routinely on thousands of electrons. For strongly correlated models, tensor networks show near-linear cost in $L$ and $\bar\epsilon \sim 1/\mathrm{poly}(D)$ in the bond dimension, even at the hard $1/8$-doped Hubbard point. The conclusion of \citet{lee2023evaluating} is that evidence for a \emph{generic} exponential advantage in ground-state chemistry has yet to be found, while polynomial speedups remain possible and would still be valuable.}
\figslide[height=0.45\textheight]{figures/eqa/fig3.pdf}{Computational complexity of classical heuristics for molecules, from \citet{lee2023evaluating}. (A) Energy error of N$_2$ against a time metric across coupled-cluster levels: $\mathrm{poly}(1/\epsilon)$. (B) Cost of reduced-scaling CCSD(T) on $n$-alkanes C$_m$H$_{2m+2}$, $m \le 120$: nearly linear in size, with constant error per carbon in the enthalpy of formation. (C) A fragment of photosystem II with thousands of electrons, treated on a few cores.}{fig:ch01:eqa3}
